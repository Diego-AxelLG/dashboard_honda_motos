"""ETL Flujos de Piso — hmcrm -> dwh.fact_flujos_piso"""
import os, sys
import pandas as pd
...
def upsert(table, conn, keys, data_iter):
    ...
    stmt = stmt.on_conflict_do_update(
        constraint="fact_flujos_piso_fecha_id_sucursal_key",
...
def main():
    ...
    try:
        ...
        sql = read_sql_file(os.path.join(PROJECT_ROOT, "etl/extract/ventas/extract_flujos_piso.sql"))
        ...
        df["freshup"] = pd.to_numeric(df["freshup"], errors="coerce").fillna(0).astype(int)
        df["internet"] = pd.to_numeric(df["internet"], errors="coerce").fillna(0).astype(int)

        df[["fecha", "id_sucursal", "freshup", "internet"]].to_sql(
            "fact_flujos_piso", pg, schema="dwh", if_exists="append", index=False, method=upsert)

        with pg.connect() as conn:
            conn.execute(text("UPDATE dwh.etl_last_run SET last_run_at=NOW() WHERE etl_name=:n"), {"n": ETL_NAME})
            conn.commit()

        logger.info(f"   {len(df):,} flujos cargados (UPSERT)")

"""ETL Presupuesto Servicio — sicofi -> dwh.fact_ppto_servicio"""
import os, sys
import pandas as pd
...
def upsert(table, conn, keys, data_iter):
    ...
    stmt = stmt.on_conflict_do_update(
        constraint="fact_ppto_servicio_fecha_id_sucursal_tipo_ppto_key",
        set_={k: getattr(stmt.excluded, k) for k in keys if k not in ("fecha", "id_sucursal", "tipo_ppto")},
    )
    conn.execute(stmt)
...
def main():
    ...
    try:
        ...
        sql = read_sql_file(os.path.join(PROJECT_ROOT, "etl/extract/postventa/extract_pptoSicofi.sql"))
        ...
        df.rename(columns={"mui": "id_sucursal", "tipoppto": "tipo_ppto", "fecha": "fecha", "planppto": "plan_ppto"}, inplace=True)
        df["id_sucursal"] = df["id_sucursal"].astype(int)
        df["fecha"] = pd.to_datetime(df["fecha"]).dt.date
        df = df.dropna(subset=["id_sucursal"])

        cols = ["fecha", "id_sucursal", "tipo_ppto", "plan_ppto"]
        df[cols].to_sql("fact_ppto_servicio", pg, schema="dwh", if_exists="append", index=False, method=upsert)

        with pg.connect() as conn:
            conn.execute(text("UPDATE dwh.etl_last_run SET last_run_at=NOW() WHERE etl_name=:n"), {"n": ETL_NAME})
            conn.commit()

        logger.info(f"   {len(df):,} ppto servicio cargados")
